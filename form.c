...
//rand_form generates a prime form 
void rand_form(mpz_t p,mpz_t D,mpz_t b,mpz_t c){
	mpz_t q,e,tmp;
	mpz_inits(q,e,tmp,NULL);
	mpz_sub_ui(q,p,1);
	mpz_set_ui(e,0);
	mpz_mod_ui(tmp,q,2);
	...
	mpz_divexact(c,c,p);
	mpz_clears(q,e,tmp,NULL);
	return;
}

int is_ambiguous(mpz_t a,mpz_t b,mpz_t c){
	if(mpz_cmp_ui(b,0) == 0) return 1;
	if(mpz_cmp(b,a) == 0) return 1;
	if(mpz_cmp(c,a) == 0) return 1;
	return 0;
}

int not_ambiguous(mpz_t a,mpz_t b,mpz_t c){
	if(mpz_cmp_ui(b,0) == 0) return 0;
	if(mpz_cmp(b,a) == 0) return 0;
	if(mpz_cmp(c,a) == 0) return 0;
	return 1;
}
...
void form_pow(mpz_t res0_,mpz_t res1_,mpz_t res2_,mpz_t a,mpz_t b,mpz_t c,mpz_t e_,unsigned int p0,mpz_t p1,mpz_t L){
	mpz_t res0,res1,res2,f0,f1,f2,e,one_,tmp;
	mpz_inits(res0,res1,res2,f0,f1,f2,e,one_,tmp,NULL);
	mpz_set_ui(res0,1);
	mpz_set_ui(res1,p0);
	...
	mpz_set(res1_,res1);
	mpz_set(res2_,res2);
  	mpz_clears(res0,res1,res2,f0,f1,f2,e,one_,tmp,NULL);
  	return;
}
